# Manuales PDF (ES)

Manuales originales en PDF de **OnPremise SRT Server**.

- [Instalación (INSTALL_SPA.pdf)](INSTALL_SPA.pdf)
- [HowTo (HOWTO_SPA.pdf)](HOWTO_SPA.pdf)
- [Características (FEATURES_SPA.pdf)](FEATURES_SPA.pdf)
- [Hardware (HARDWARE_SPA.pdf)](HARDWARE_SPA.pdf)
- [Multi-red / multi-NIC (MULTI-NET_SPA.pdf)](MULTI-NET_SPA.pdf)
- [API (API_ES.pdf)](API_ES.pdf)
- [SRT → UDP/RTP para cabeceras de cable (SRT_to_UDP-RTP_for_cable_headends_SPA.pdf)](SRT_to_UDP-RTP_for_cable_headends_SPA.pdf)
- [Vídeos (ES)](../videos/index.md)

## Volver
- [Docs ES (inicio)](../../index.md)
- [Docs (selector de idioma)](../../../index.md)
